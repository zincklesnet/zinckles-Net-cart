...
class ZNC_Network_Admin {
    ...
    private static $defaults = array(
        ...
        // ── Global Defaults ─────────────────────────────────────
        'base_currency'         => 'CAD',            // network base currency
        'allow_mixed_currency'  => true,
        'mycred_enabled'        => true,
        'mycred_point_type'     => 'zcreds',
        'mycred_exchange_rate'  => 1.0,              // 1 ZCred = $1.00 base
        'mycred_max_percent'    => 50,               // max % of order payable in ZCreds
        ...
        'debug_mode'            => false,            // shows REST payloads in admin
    );
    ...
    public static function init() {
        ...
        add_action( 'network_admin_edit_action_znc_save_network', array( __CLASS__, 'save_settings' ) );
    ...
    /**
     * Check if a blog_id is enrolled in Net Cart.
     */
    public static function is_site_enrolled( $blog_id ) {
        $settings = self::get_settings();

        // Explicitly blocked always wins.
        if ( in_array( (int) $blog_id, (array) $settings['blocked_sites'], true ) ) {
            return false;
        }

        switch ( $settings['enrollment_mode'] ) {
            case 'opt-out':
                // All sites enrolled unless blocked.
                return true;

            case 'manual':
            case 'opt-in':
            default:
                return in_array( (int) $blog_id, (array) $settings['enrolled_sites'], true );
        }
    }

    /**
     * Get all enrolled subsites with metadata.
     */
    ...
    /**
     * Enroll or unenroll a site.
     */
    public static function set_site_enrollment( $blog_id, $enrolled ) {
        $settings = self::get_settings();
        $blog_id  = (int) $blog_id;

        $settings['enrolled_sites'] = array_values( array_diff( (array) $settings['enrolled_sites'], array( $blog_id ) ) );
        $settings['blocked_sites']  = array_values( array_diff( (array) $settings['blocked_sites'], array( $blog_id ) ) );
    ...
    /**
     * Generate and store a new REST shared secret.
     */
    ...
    public static function ajax_toggle_site() {
        ...
        $enrolled = ! empty( $_POST['enrolled'] );
        self::set_site_enrollment( $blog_id, $enrolled );
        wp_send_json_success( array(
            'blog_id'  => $blog_id,
            'enrolled' => $enrolled,
        ) );
    ...
    public static function save_settings() {
        ...
        // Enrollment.
        $clean['enrollment_mode'] = in_array( $input['enrollment_mode'] ?? '', array( 'opt-in', 'opt-out', 'manual' ), true )
        ...
        $clean['mycred_point_type']    = sanitize_key( $input['mycred_point_type'] ?? 'zcreds' );
        ...
        // Checkout.
        $clean['checkout_validation']   = in_array( $input['checkout_validation'] ?? '', array( 'strict', 'relaxed' ), true )
        ...
        $clean['log_level']          = in_array( $input['log_level'] ?? '', array( 'debug', 'info', 'warning', 'error' ), true )
    ...
    private static function count_products() {
        ...
        return count( wc_get_products( array( 'status' => 'publish', 'limit' => -1, 'return' => 'ids' ) ) );
    }
}
